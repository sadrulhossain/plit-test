...
//function for getOrderList
    ...
    public static function getInDepoProduct($productId, $skuId) {
        $total = 0;
        if (!empty(Auth::user()->id) && Auth::user()->group_id == 14) {
            $warehouseInfo = WarehouseToSr::select('warehouse_id')->where('sr_id', Auth::user()->id)->first();
            $inDepoProductsArr = WarehouseStore::join('product_sku_code', 'product_sku_code.id', 'wh_store.sku_id')
                            ->where('wh_store.warehouse_id', $warehouseInfo->warehouse_id)
                            ->where('product_sku_code.product_id', $productId)
                            ->where('wh_store.sku_id', $skuId)
                            ->select(DB::raw('COUNT(wh_store.id) as total'))->first();
            $total = $inDepoProductsArr->total;
        }
        if (!empty(Auth::user()->id) && in_array(Auth::user()->group_id,[18,19]) ) {
            ...
            $inDepoProductsArr = WarehouseStore::join('product_sku_code', 'product_sku_code.id', 'wh_store.sku_id')
                            ...
                            ->where('product_sku_code.product_id', $productId)
                            ->where('wh_store.sku_id', $skuId)
                            ->select(DB::raw('COUNT(wh_store.id) as total'))
                            ->first();
//            echo json_encode($inDepoProductsArr);exit;
            $total = $inDepoProductsArr->total;
        }
